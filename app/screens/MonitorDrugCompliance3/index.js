...
function MonitorDrugCompliance3({ navigation, user }) {
  ...
  const fetchTime = value => {
    ...
    if (user?.data?.userInformation?.drugTime?.evening) {
      let beginTime = user?.data?.userInformation?.drugTime?.evening?.begin;
      let endTime = user?.data?.userInformation?.drugTime?.evening?.end;
  ...
  const checkTaken = async (token, beforeList, afterList, time) => {
    try {
      ...
      if (user?.data?.userInformation?.drugTime?.evening) {
        let beginTime = user?.data?.userInformation?.drugTime?.evening?.begin;
        let eatTime = user?.data?.userInformation?.drugTime?.evening?.eat;
        let endTime = user?.data?.userInformation?.drugTime?.evening?.end;
    ...
    try {
      ...
      if (data !== null) {
        ...
        if (available.length > 0) {
          ...
          setBefore(eveningBefore);
          setAfter(eveningAfter);
  ...
  useEffect(() => {
    fetchDrugs();
    const evening = user?.data?.userInformation?.drugTime?.evening;

    if (evening) {
      setTempTime({
        begin: moment(evening.begin).format('HH:mm'),
        end: moment(evening.end).format('HH:mm'),
        eat: moment(evening.eat).format('HH:mm'),
      });
    }
  }, []);
...
export default connect(mapStateToProps, null)(MonitorDrugCompliance3);
